...
export default function OpportunitiesPage() {
  ...
  const queryKey = useMemo(
    () => ['opportunities', filters, pagination.cursor, pagination.limit],
  ...
  const opportunitiesQuery = useQuery({
    ...
    queryFn: async () => {
      setError(null);
      try {
        return await apiClient<PaginatedResponse<OpportunityRow>>('opportunities', {
          token,
          searchParams: {
            ...filters,
  ...
  const metrics = useMemo(() => {
    // Coerce numeric fields to numbers to avoid concatenation/NaN
    const toNumber = (v: unknown): number => {
      const n = typeof v === 'number' ? v : Number(v);
      return Number.isFinite(n) ? n : 0;
    };
    // Exclude closed_lost from pipeline value calculation
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold text-slate-900">Opportunities Pipeline</h2>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-xs font-medium text-slate-500">Active Pipeline Value</p>
      ...
      {showCreateForm && (
        <form className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-3" onSubmit={onSubmit}>
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-slate-600" htmlFor="referral_id">
              Referral Code (optional)
            </label>
            <select
              id="referral_id"
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              {...form.register('referral_id')}
            >
                  ...
                  <div className="space-y-2">
                    {stageOpps.map((opp) => (
                      <div key={opp.id} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm hover:shadow-md transition-shadow">
                        <Link to={`/portfolio/${opp.company_id}`} className="font-medium text-primary hover:underline">
                          {opp.title}
                        </Link>
                        ...
                        {opp.owner_id && (
                          <p className="mt-1 text-xs text-slate-500">Owner: {getOwnerName(opp.owner_id)}</p>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            columns={[
              {
                ...
                cell: (opp: OpportunityRow) => (
                  <div>
                    ...
                    <Link to={`/portfolio/${opp.company_id}`} className="text-xs text-primary hover:underline">
              ...
              { id: 'owner', header: 'Owner', cell: (opp: OpportunityRow) => getOwnerName(opp.owner_id) },
              {
                id: 'expected_close',
      ...
      >
        <form onSubmit={onEditSubmit} className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-600" htmlFor="edit_currency">
                Currency
              </label>
              <select
                id="edit_currency"
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-600" htmlFor="edit_referral_id">
                Referral
              </label>
              <select
                id="edit_referral_id"
